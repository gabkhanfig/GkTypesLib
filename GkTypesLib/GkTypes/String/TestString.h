...
#pragma region Construct_Destruct
		...
		constexpr TestString() : _rep(StringRep()), _flag(0) {
			setSsoUsedBytes(0);
		}

		constexpr TestString(const char c) : _rep(StringRep()), _flag(1) {
			_rep._sso._chars[0] = c;
			setSsoUsedBytes(1);		
		}

		constexpr TestString(const Str& str) {
		...
		constexpr TestString(const TestString& other) {
			if (other.isSso()) {
				if (std::is_constant_evaluated()) {
					for (int i = 0; i < 31; i++) {
				...
				memcpy(this, &other, sizeof(TestString));
				return;
			}
		...
		constexpr TestString(TestString&& other) noexcept {
			if (other.isSso()) {
				if (std::is_constant_evaluated()) {
					for (int i = 0; i < 31; i++) {
						_rep._sso._chars[i] = other._rep._sso._chars[i];
...
#pragma region Assign
		...
		constexpr TestString& operator = (const char c) {
			freeHeapBufferIfNotSso();
			_rep._sso = SsoRep();
			_rep._sso._chars[0] = c;
			_flag = 1;
		...
		constexpr TestString& operator = (const Str& str) {
			const uint64 stringBytes = str.totalBytes - 1;
		...
		constexpr TestString& operator = (const TestString& other) {
		...
		constexpr TestString& operator = (TestString&& other) noexcept {
			freeHeapBufferIfNotSso(); 
			if (other.isSso()) {
				if (std::is_constant_evaluated()) {
					for (int i = 0; i < 31; i++) {
...
#pragma endregion
		...
		/* std::cout << string */
		friend std::ostream& operator << (std::ostream& os, const TestString& _string) {
...
#pragma region Compare
		...
		[[nodiscard]] constexpr bool operator == (const TestString& other) const {
			if (std::is_constant_evaluated()) {
...
#pragma region Append
		...
		constexpr TestString& append(const char c) {
			const uint64 length = len();
		...
		constexpr TestString& append(const Str& str) {
			const uint64 length = len();
		...
		constexpr TestString& append(const TestString& other) {
			const uint64 length = len();
		...
		constexpr forceinline TestString& operator += (const char c)						{ return append(c); }
		constexpr forceinline TestString& operator += (const Str& str)					{ return append(str); }
		constexpr forceinline TestString& operator += (const TestString& other) { return append(other); }

#pragma endregion

#pragma region Concat

		[[nodiscard]] constexpr friend TestString operator + (const TestString& lhs, const char rhs) {
			...
			char* buffer = TestString::mallocHeapBuffer(&mallocCapacity);
		...
		[[nodiscard]] constexpr friend TestString operator + (TestString&& lhs, const char rhs) { return lhs.append(rhs); }

		[[nodiscard]] constexpr friend TestString operator + (const TestString& lhs, const Str& rhs) {
			const uint64 length = lhs.len();
		...
		[[nodiscard]] constexpr friend TestString operator + (TestString&& lhs, const Str& rhs) { return lhs.append(rhs); }

		[[nodiscard]] constexpr friend TestString operator + (const TestString& lhs, const TestString& rhs) {
			const uint64 length = lhs.len();
			const uint64 newLength = length + rhs.len();
			const uint64 capacityUsedNoTerminator = lhs.usedBytes();
		...
		[[nodiscard]] constexpr friend TestString operator + (TestString&& lhs, const TestString& rhs) { return lhs.append(rhs); }

		[[nodiscard]] constexpr friend TestString operator + (const char lhs, const TestString& rhs) {
		...
		[[nodiscard]] constexpr friend TestString operator + (const Str& lhs, const TestString& rhs) {
			const uint64 length = rhs.len();
...
#pragma region From_Other_Type
		...
		[[nodiscard]] constexpr static TestString fromBool(const bool b) {
			TestString newString;
			if (b) {
		...
		[[nodiscard]] constexpr static TestString fromInt(int64 num) {
			if (num == 0) return TestString('0');
			...
			TestString newString; // Only needs 20 + 1 characters max, so is guaranteed to fit in sso buffer
		...
		[[nodiscard]] constexpr static TestString fromUint(uint64 num) {
			if (num == 0) return TestString('0');
		...
		[[nodiscard]] constexpr static TestString fromFloat(double num, const int precision = 5) {
			gk_assertm(precision < 20, "gk::String::FromFloat precision must be 19 or less. The precision is set to " << precision);

			if (num == 0)							return TestString("0.0");			// Zero
			else if (num > DBL_MAX)		return TestString("inf");			// Positive Infinity
			else if (num < -DBL_MAX)	return TestString("-inf");		// Negative Infinity
			else if (num != num)			return TestString("nan");			// Not a Number

			_StripNegativeZero(num);
			const bool isNegative = num < 0;

			const int64 whole = static_cast<int64>(num);
			TestString wholeString = (isNegative && whole == 0) ? gk::Str("-0") : TestString::fromInt(whole);
			...
			auto MakeStringWithIntWithoutEndingZeroes = [](const uint64 value, const int availableDigits) {
				TestString str = TestString::fromUint(value);
			...
			const TestString fractionZeroesString = MakeFractionZeroesString(zeroesInFractionBeforeFirstNonZero);
			const TestString fractionalString = MakeStringWithIntWithoutEndingZeroes(fraction, precision - zeroesInFractionBeforeFirstNonZero);

			return wholeString + '.' + fractionZeroesString + fractionalString;
		}

		template<typename T>
		[[nodiscard]] constexpr static TestString from(const T& value) = delete;

		template<>
		[[nodiscard]] constexpr static TestString from<bool>(const bool& b) { return fromBool(b); }

		template<>
		[[nodiscard]] constexpr static TestString from<int8>(const int8& num) { return fromInt(num); }

		template<>
		[[nodiscard]] constexpr static TestString from<int16>(const int16& num) { return fromInt(num); }

		template<>
		[[nodiscard]] constexpr static TestString from<int>(const int& num) { return fromInt(num); }

		template<>
		[[nodiscard]] constexpr static TestString from<int64>(const int64& num) { return fromInt(num); }

		template<>
		[[nodiscard]] constexpr static TestString from<uint8>(const uint8& num) { return fromUint(num); }

		template<>
		[[nodiscard]] constexpr static TestString from<uint16>(const uint16& num) { return fromUint(num); }

		template<>
		[[nodiscard]] constexpr static TestString from<uint32>(const uint32& num) { return fromUint(num); }

		template<>
		[[nodiscard]] constexpr static TestString from<uint64>(const uint64& num) { return fromUint(num); }

		template<>
		[[nodiscard]] constexpr static TestString from<float>(const float& num) { return fromFloat(num); }

		template<>
		[[nodiscard]] constexpr static TestString from<double>(const double& num) { return fromFloat(num); }
...
#pragma region Format		
		...
		*/
		constexpr static TestString format(const Types&... inputs) {
			...
			TestString argsAsStrings[argumentCount];
			...
			([&] // loop through all elements, checking the types
				{
					if constexpr (std::is_same_v<Types, gk::TestString>) {
						...
						argsAsStrings[i] = TestString::formatStrConvertArg<Types>(inputs);
...
#pragma region Find
		...
		/* Find the index of a specified substring in the cstr array. */
		constexpr Option<StringIndex> find(const TestString& other) const {
			const uint64 length = len();
			const uint64 otherLen = other.len();
...
#pragma region Substring
		...
		[[nodiscard]] constexpr TestString substring(const uint64 startIndexInclusive, const uint64 endIndexExclusive) const {
...
#pragma region Comparison_Dynamic_Dispatch
		...
		typedef bool (*FuncCmpEqStringAndStr)(const TestString&, const Str&);
		typedef bool (*FuncCmpEqStringAndString)(const TestString&, const TestString&);

		[[nodiscard]] static bool avx512CompareEqualHeapBufferAndStr(const TestString& self, const Str& str) {
		...
		[[nodiscard]] static bool avx512CompareEqualHeapBufferAndOtherStringHeapBuffer(const TestString& self, const TestString& other) {
			constexpr uint64 equal64Bitmask = ~0;
			const __m512i* thisVec = reinterpret_cast<const __m512i*>(self._rep._heap._buffer);
		...
		[[nodiscard]] static bool avx2CompareEqualHeapBufferAndStr(const TestString& self, const Str& str) {
		...
		[[nodiscard]] static bool avx2CompareEqualHeapBufferAndOtherStringHeapBuffer(const TestString& self, const TestString& other) {
			constexpr uint32 equal32Bitmask = ~0;
			const __m256i* thisVec = reinterpret_cast<const __m256i*>(self._rep._heap._buffer);
...
#pragma region Find_Dynamic_Dispatch
		...
		typedef Option<StringIndex>(*FuncFindCharInString)(const TestString&, const char);

#pragma intrinsic(_BitScanForward64)
#pragma intrinsic(_BitScanForward)

		[[nodiscard]] static Option<StringIndex> avx512FindCharInString(const TestString& self, const char c) {
		...
		[[nodiscard]] static Option<StringIndex> avx2FindCharInString(const TestString& self, const char c) {
...
#pragma region Format_Functionality
			...
			template<typename T>
			constexpr static TestString formatStrConvertArg(const T& arg) {
				auto convertCheck = [](auto& t) -> decltype(TestString::from<T>(t)) {
					return TestString::from<T>(t);
				};
...
{
	template<>
	struct hash<gk::TestString>
	{
		size_t operator()(const gk::TestString& str) const {
			return str.hash();
		}
	};
}